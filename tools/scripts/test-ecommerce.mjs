...
async function testAuthentication() {
  ...
  const wrongPass = await gql(`mutation { customerLogin(email: "marie.dupont@gmail.com", password: "wrong") { accessToken } }`);
  logTest('Reject invalid password', !!wrongPass.errors);

  const noUser = await gql(`mutation { customerLogin(email: "fake@test.com", password: "test") { accessToken } }`);
...
async function testCategories() {
  ...
  await test('List categories', `query { categories { id name } }`, tokens.admin, d => d.categories?.length >= 5);
  await test('Get category tree', `query { categoryTree { id name active } }`, tokens.admin, d => d.categoryTree?.length >= 5);
  ...
  if (cat?.createCategory?.id) {
    ...
    // Read
    await test('Get category by ID', `query { category(id: "${testData.categoryId}") { id name } }`, tokens.admin, d => !!d.category?.id);
    await test('Get category path', `query { categoryPath(id: "${testData.categoryId}") }`, tokens.admin);
...
async function testAttributes() {
  ...
  await test('List attribute groups', `query { attributeGroups { id name values { id name } } }`, tokens.admin, d => d.attributeGroups?.length >= 2);
  ...
  if (grp?.createAttributeGroup?.id) {
    ...
    // Get group
    await test('Get attribute group', `query { attributeGroup(id: "${testData.attrGroupId}") { id name } }`, tokens.admin, d => !!d.attributeGroup?.id);
...
async function testProducts() {
  ...
  await test('List products', `query { products(filter: { take: 20 }) { items { id name } total } }`, tokens.admin, d => d.products?.total >= 20);
  ...
  if (prod?.createProduct?.id) {
    ...
    // Read
    await test('Get product by ID', `query { product(id: "${testData.productId}") { id name price } }`, tokens.admin, d => !!d.product?.id);
    ...
    if (img?.addProductImage?.id) {
      ...
      await test('Set cover image', `mutation { setProductCoverImage(imageId: "${testData.imageId}") { id cover } }`, tokens.admin, d => d.setProductCoverImage?.cover === true);
    }
    ...
    if (stk?.updateStock?.id) {
      ...
      await test('Increment stock +50', `mutation { incrementStock(stockId: "${testData.stockId}", quantity: 50) { id quantity } }`, tokens.admin, d => d.incrementStock?.quantity === 150);
      await test('Decrement stock -25', `mutation { decrementStock(stockId: "${testData.stockId}", quantity: 25) { id quantity } }`, tokens.admin, d => d.decrementStock?.quantity === 125);
      await test('Check availability', `query { checkProductAvailability(productId: "${testData.productId}", quantity: 10) }`, null, d => d.checkProductAvailability === true);
    }
...
async function testPricing() {
  ...
  await test('List countries', `query { countries { id isoCode name taxRate } }`, tokens.admin, d => d.countries?.length >= 8);

  // Get product and country for calculatePrice
  const prods = await gql(`query { products(filter: { take: 1 }) { items { id } } }`, tokens.admin);
  const countries = await gql(`query { countries { id isoCode } }`, tokens.admin);
  
  if (prods.data?.products?.items?.[0] && countries.data?.countries?.[0]) {
    const productId = prods.data.products.items[0].id;
    const countryId = countries.data.countries.find(c => c.isoCode === 'FR')?.id || countries.data.countries[0].id;

    await test(
      'Calculate price (with tax)',
      `query { calculatePrice(productId: "${productId}", countryId: "${countryId}") { basePrice taxRate taxAmount priceTTC } }`,
      null,
      d => !!d.calculatePrice?.priceTTC && parseFloat(d.calculatePrice.priceTTC) > 0
    );
  }

  // Specific prices
  await test('List specific prices', `query { specificPrices { id reduction reductionType productId } }`, tokens.admin, d => Array.isArray(d.specificPrices));
...
async function testCustomerGroups() {
  ...
  const groups = await test('List customer groups', `query { customerGroups { id name reduction } }`, tokens.admin, d => d.customerGroups?.length >= 4);
  
  if (groups) {
    logTest('VIP group has 10% reduction', groups.customerGroups.some(g => g.name === 'VIP' && parseFloat(g.reduction) === 10));
    logTest('Wholesale has 15% reduction', groups.customerGroups.some(g => g.name === 'Wholesale' && parseFloat(g.reduction) === 15));
  }

  await test('List all customers', `query { customers { id email group { name } } }`, tokens.admin, d => d.customers?.length >= 15);
}
...
async function testAddresses() {
  ...
  const countries = await gql(`query { countries { id isoCode } }`, tokens.admin);
  const countryId = countries.data?.countries?.find(c => c.isoCode === 'FR')?.id;

  if (countryId) {
    await test('List my addresses', `query { myAddresses { id alias city } }`, tokens.customer);
    ...
    if (addr?.createAddress?.id) {
      ...
      await test('Get address by ID', `query { address(id: "${testData.addressId}") { id alias } }`, tokens.customer, d => !!d.address?.id);
  ...
  // Admin list all
  await test('Admin list all addresses', `query { addresses { id alias customerId } }`, tokens.admin);
}
...
async function testSecurity() {
  ...
  const noAuth = await gql(`mutation { createProduct(input: { reference: "HACK", name: "Hacked", price: 0 }) { id } }`);
  logTest('No auth cannot create product', !!noAuth.errors);

  if (testData.productId) {
    const custDel = await gql(`mutation { deleteProduct(id: "${testData.productId}") { id } }`, tokens.customer);
...
async function cleanup() {
  ...
  if (testData.addressId) await test('Delete address', `mutation { deleteAddress(id: "${testData.addressId}") { id } }`, tokens.customer);
  if (testData.specificPriceId) await test('Delete specific price', `mutation { deleteSpecificPrice(id: "${testData.specificPriceId}") { id } }`, tokens.admin);
  if (testData.combinationId) await test('Delete combination', `mutation { deleteProductCombination(id: "${testData.combinationId}") { id } }`, tokens.admin);
  if (testData.imageId) await test('Remove image', `mutation { removeProductImage(id: "${testData.imageId}") { id } }`, tokens.admin);
  if (testData.productId) await test('Delete product', `mutation { deleteProduct(id: "${testData.productId}") { id } }`, tokens.admin);
  if (testData.attrValueId) await test('Delete attribute value', `mutation { deleteAttributeValue(id: "${testData.attrValueId}") { id } }`, tokens.admin);
  if (testData.attrGroupId) await test('Delete attribute group', `mutation { deleteAttributeGroup(id: "${testData.attrGroupId}") { id } }`, tokens.admin);
  if (testData.categoryId) await test('Delete category', `mutation { deleteCategory(id: "${testData.categoryId}") { id } }`, tokens.admin);
}
...
async function main() {
  ...
  log(`Success: ${rate}%`, rate >= 95 ? 'green' : rate >= 80 ? 'yellow' : 'red');
  log('═'.repeat(60) + '\n', 'magenta');
