...
async function runTests() {
  ...
  const loginData = await gql(
    `mutation { customerLogin(email: "marie.dupont@gmail.com", password: "customer123") { accessToken customer { id } } }`,
  );
  ...
  const productsRes = await gql(
    `query { products { items { id name combinations { id } } } }`,
  );
  if (
    productsRes &&
    productsRes.products &&
    productsRes.products.items.length > 0
  ) {
  ...
  if (addressData && addressData.myAddresses.length > 0) {
    ...
    const newAddr = await gql(
      `mutation { createAddress(input: { alias: "Test", firstname: "Test", lastname: "Test", address1: "123 Rue", postcode: "75000", city: "Paris", countryId: "${countryId}" }) { id } }`,
      customerToken,
    );
  ...
  if (
    await test(
      'Add to Cart',
      `mutation { addToCart(input: { productId: "${productId}", quantity: 2${
        combinationId ? `, combinationId: "${combinationId}"` : ''
      } }) { id items { quantity product { name } lineTotal } totalTTC } }`,
      customerToken,
      (d) =>
        d.addToCart &&
        d.addToCart.items.length === 1 &&
        d.addToCart.items[0].quantity === 2,
    )
  )
    passed++;
  ...
  if (
    await test(
      ...
      `mutation { addToCart(input: { productId: "${productId}", quantity: 1${
        combinationId ? `, combinationId: "${combinationId}"` : ''
      } }) { items { quantity } } }`,
      customerToken,
  ...
  if (
    await test(
      ...
      (d) =>
        d.createOrder &&
        d.createOrder.reference.startsWith('DO-') &&
        d.createOrder.state.name === 'PENDING',
    )
  )
    passed++;
  ...
  const lastOrderId = (await gql(`query { myOrders { id } }`, customerToken))
    .myOrders[0].id;
  ...
  log(
    `Failed:  ${total - passed}`,
    total - passed > 0 ? COLORS.red : COLORS.green,
  );
