...
(async () => {
  ...
  `, token);
  log('👤', 'Employee Me', !meRes.errors, meRes.data?.employeeMe || meRes.errors);
  
  // 3. Roles CRUD
  console.log('\n=== 2. Roles Management ===');
  
  // List Roles
  const rolesRes = await gql('query { roles { id name description isSystem } }', token);
  log('📋', 'List Roles', !rolesRes.errors, rolesRes.data?.roles || rolesRes.errors);
  ...
  `, token);
  log('➕', 'Create Role', !createRoleRes.errors, createRoleRes.data?.createRole || createRoleRes.errors);
  ...
  if (roleId) {
    ...
    `, token);
    log('✏️', 'Update Role', !updateRoleRes.errors, updateRoleRes.data?.updateRole || updateRoleRes.errors);
    ...
    `, token);
    log('🗑️', 'Delete Role', !deleteRoleRes.errors, deleteRoleRes.data?.deleteRole || deleteRoleRes.errors);
  }
  ...
  `, token);
  log('📋', 'List Employees', !employeesRes.errors, employeesRes.data?.employees || employeesRes.errors);
  ...
  if (salesRole) {
    ...
    `, token);
    log('➕', 'Create Employee', !createEmpRes.errors, createEmpRes.data?.createEmployee || createEmpRes.errors);
    ...
    if (empId) {
      ...
      `, token);
      log('🔍', 'Get Employee', !getEmpRes.errors, getEmpRes.data?.employee || getEmpRes.errors);
      ...
      `, token);
      log('✏️', 'Update Employee', !updateEmpRes.errors, updateEmpRes.data?.updateEmployee || updateEmpRes.errors);
      ...
      `, token);
      log('🗑️', 'Delete Employee', !deleteEmpRes.errors, deleteEmpRes.data?.deleteEmployee || deleteEmpRes.errors);
    }
  }
  ...
  if (customerToken) {
    ...
    `, customerToken);
    log('👤', 'Customer Me', !customerMeRes.errors, customerMeRes.data?.customerMe || customerMeRes.errors);
  }
  ...
  `);
  log('📝', 'Customer Register', !registerRes.errors, registerRes.data?.customerRegister?.customer || registerRes.errors);
  
  // 6. Customer Groups
  console.log('\n=== 5. Customer Groups ===');
  
  // List Groups
  const groupsRes = await gql('query { customerGroups { id name reduction showPrices } }', token);
  log('📋', 'List Groups', !groupsRes.errors, groupsRes.data?.customerGroups || groupsRes.errors);
  ...
  `, token);
  log('➕', 'Create Group', !createGroupRes.errors, createGroupRes.data?.createCustomerGroup || createGroupRes.errors);
  ...
  if (groupId) {
    ...
    `, token);
    log('🔍', 'Get Group', !getGroupRes.errors, getGroupRes.data?.customerGroup || getGroupRes.errors);
    ...
    `, token);
    log('✏️', 'Update Group', !updateGroupRes.errors, updateGroupRes.data?.updateCustomerGroup || updateGroupRes.errors);
    ...
    `, token);
    log('🗑️', 'Delete Group', !deleteGroupRes.errors, deleteGroupRes.data?.deleteCustomerGroup || deleteGroupRes.errors);
  }
